...
impl InitShape {
    ...
    pub fn modal_ui(
        ...
        let mut end_operation = InitShapeModalEndOperation::None;
        {
            ...
            Modal::new(Id::new("Init Shape Modal")).show(ui.ctx(), |ui| {
                ...
                if modal_ui.use_lifetimes {
                    Grid::new("Init Shape Grid Lifetime Range")
                        ...
                        .show(ui, |ui| {
                            ui.label("Min");
                            ui.add(
                                egui::DragValue::new(match &mut modal_ui.descriptor {
                                    InitShapeDescriptor::Cube {
                                        starting_lifetime, ..
                                    } => &mut starting_lifetime[0],
                                    InitShapeDescriptor::Sphere {
                                        starting_lifetime, ..
                                    } => &mut starting_lifetime[0],
                                })
                ...
                if let InitShapeDescriptor::Cube { spawn_density, .. } = &mut modal_ui.descriptor {
                    ui.horizontal(|ui| {
                        ...
                        ui.add(
                            egui::DragValue::new(spawn_density)
                                .speed(1)
                                .range(1..=u32::MAX),
                        );
                    });
