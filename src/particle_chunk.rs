...
impl ParticleChunk {
    pub fn new(
        ...
        loop {
            ...
            particle_count_z = std::cmp::min(
                std::cmp::max(
                    particle_count_goal / (particle_count_x * particle_count_y),
                    1,
                ),
                dimension_limit,
            );
            ...
            particle_count_goal = std::cmp::min(
                particle_count_goal,
                device.limits().max_storage_buffer_binding_size as u32
                    / std::mem::size_of::<ParticleLifetime>() as u32,
            );
        }
        info!(
            "Creating particle chunk with {} particles ({} x {} x {})",
            particle_count_total, particle_count_x, particle_count_y, particle_count_z
        );
        assert!(
            particle_count_total <= particle_count_goal,
            "Particle count total exceeds goal"
        );
        assert!(
            particle_count_x % 64 == 0,
            "Particle count x must be divisible by 64"
        );
        assert!(
            particle_count_x <= dimension_limit,
            "Particle count x exceeds dimension limit"
        );
        assert!(
            particle_count_y <= dimension_limit,
            "Particle count y exceeds dimension limit"
        );
        assert!(
            particle_count_z <= dimension_limit,
            "Particle count z exceeds dimension limit"
        );
        assert!(
            particle_count_total
                <= device.limits().max_storage_buffer_binding_size as u32
                    / std::mem::size_of::<ParticleVertex>() as u32,
            "Particle count total exceeds max storage buffer binding size"
        );
    ...
    pub fn dispatch_update(&self, pass: &mut wgpu::ComputePass<'_>) {
        pass.set_bind_group(0, &self.bind_group, &[]);
        pass.dispatch_workgroups(
            self.particle_count_x / 64,
            self.particle_count_y,
            self.particle_count_z,
        );
    }
}
