...
impl Renderers {
    pub fn new(
        ...
        Self {
            points_renderer: PointsRenderer::new(
                device,
                simulation_params_group_layout,
                view_proj_group_layout,
                config,
            ),
        }
    }

    pub fn get_renderer(&self, renderer_type: RendererType) -> &dyn Renderer {
        match renderer_type {
            RendererType::Points => &self.points_renderer,
        }
    }

    pub fn get_points_renderer(&mut self) -> &mut PointsRenderer {
